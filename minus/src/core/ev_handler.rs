    ...
    match ev {
        ...
        Command::UserInput(InputEvent::Search(m)) => {
            ...
            *active = true;
            ...
            // Handle repeat search (// or ??)
            if search_result.is_repeat_search {
                // Use the previous search term if available
                if let Some(prev_term) = &p.search_state.previous_search_term {
                    p.search_state.search_term = Some(prev_term.clone());
                    // Format the lines with the previous search term
                    p.format_lines();

                    // After formatting, move to the next/previous match based on search mode
                    if p.search_state.search_mode == search::SearchMode::Forward {
                        // For forward search, move to next match
                        // If we're at the last match, wrap around to the first match
                        let current_mark = p.search_state.search_mark;
                        let total_matches = p.search_state.search_idx.len();

                        if current_mark >= total_matches.saturating_sub(1) {
                            // We're at or past the last match, wrap to the first match
                            if !p.search_state.search_idx.is_empty() {
                                p.search_state.search_mark = 0;
                                let upper_mark = *p.search_state.search_idx.iter().next().unwrap();
                    ...
                    } else if p.search_state.search_mode == search::SearchMode::Reverse {
                        // For reverse search, move to previous match
                        // If we're at the first match, wrap around to the last match
                        let current_mark = p.search_state.search_mark;
                        let total_matches = p.search_state.search_idx.len();

                        if current_mark == 0 || total_matches == 0 {
                            // We're at the first match or no matches, wrap to the last match
                            if !p.search_state.search_idx.is_empty() {
                                p.search_state.search_mark = total_matches.saturating_sub(1);
                                let upper_mark = *p.search_state.search_idx.iter().last().unwrap();
                                command_queue.push_back_unchecked(Command::UserInput(
                                    InputEvent::UpdateUpperMark(upper_mark),
                                ));
                                command_queue.push_back_unchecked(Command::FormatRedrawPrompt);
                            ...
                            p.search_state.search_mark =
                                p.search_state.search_mark.saturating_sub(1);
                            if let Some(y) = p
                                .search_state
                                .search_idx
                                .iter()
                    ...
                    // No previous search term available
                    command_queue.push_back_unchecked(Command::SendMessage(
                        "No previous search term. Press Enter".to_string(),
                    ));
                    return Ok(());
                }
            }
            ...
            if let Some(incremental_search_result) = search_result.incremental_search_result {
                ...
                // Store the current search term as the previous search term for future repeat searches
                // Only store if we have a valid search term and it's not a repeat search
                if !search_result.is_repeat_search {
                    if let Some(ref current_term) = p.search_state.search_term {
                        p.search_state.previous_search_term = Some(current_term.clone());
                    }
                }
